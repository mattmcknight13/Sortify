...
def sort_album():
    ...
    sorted_album['duration_ms'] = [f'{str(int(time//60))}:0{str(floor(time % 60))}' if len(str(floor(time % 60))) == 1 else f'{str(int(time//60))}:{str(floor(time % 60))}' for time in (sorted_album['duration_ms'] * 0.001)]
    sorted_album['index'] = sorted_album['index'] + 1

    sorted_album.rename(columns = {'index':'Index', 'track':'Track', 'duration_ms':'Duration'}, inplace = True)
...
def model(album, sp):
    ...
    album_tracks, album_final_model, album_info_dict = get_album_tracks(album, sp)
